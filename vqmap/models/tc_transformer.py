...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.sr_ratio > 1:
            x_ = x.permute(0, 2, 1)#.reshape(B, C, -1)
...
class TokenConv(nn.Conv1d):
    ...
    def forward(self, token_dict):
        ...
        x_map = x_map.flatten(2)
        
        x_map = super().forward(x_map)
        
        x_map = x_map.unsqueeze(2)
        
        x_map = map2token(x_map, token_dict)
...
class CTM(nn.Module):
    ...
    def forward(self, token_dict):
        ...
        T = token_dict['map_size'][1]
        T = math.floor((T - 1) / 4 + 1)
        down_dict['map_size'] = [1, T]
        # print(down_dict['map_size'], down_dict['x'].shape)
...
class TCFormer1D(nn.Module):
    def __init__(
        self, seqlen=256, in_chans=69,
        ...
        **kwargs
        ...
        for i in range(1):
            patch_embed = ConvNormRelu(in_chans, embed_dims[0], leaky=True, sample='down')
            block =  nn.ModuleList([Block(
                dim=embed_dims[i], num_heads=num_heads[i], mlp_ratio=mlp_ratios[i], qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, drop_path=dpr[cur + j], norm_layer=norm_layer,
                sr_ratio=sr_ratios[i])
    ...
    def forward_features(self, x):
        ...
        norm = getattr(self, f"norm{i + 1}")
        x = patch_embed(x)
        print("Init embed: ", x.shape)

        # TODO: problem here with 2D conv in the MLP
        x = x.permute(0, 2, 1)
        for blk in block:
            x = blk(x)
        x = norm(x)
        print("Before token: ", x.shape)
...
if __name__ == "__main__":
    ...
    T = 256
    C = 69
    inputs = torch.randn(B, C, T)
    model = TCFormer1D()
    outputs = model(inputs)
    
    for output in outputs:
        print("Feature map: ", output['x'].shape, "token_num: ", output['token_num'])
