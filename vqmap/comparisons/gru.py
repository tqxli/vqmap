...
def augment_x(x, mask, lengths, concatenate_time):
    ...
    if concatenate_time:
        ...
        time = time[..., None]
        x_augmented = torch.cat((x, time), 2)
    else:
        x_augmented = x #torch.cat((x, y), 2)
    return x_augmented


def augment_z(z, mask, lengths, concatenate_time):
    # if len(y.shape) == 1:  # can give on hot encoded as input
    #     y = F.one_hot(y, num_classes)
    # y = y.to(dtype=z.dtype)
    # concatenete z and y and repeat the input
    # z_augmented = torch.cat((z, y), 1)[:, None].repeat((1, mask.shape[1], 1))
    z_augmented = z[:, None].repeat((1, mask.shape[1], 1))
...
class Encoder_GRU(nn.Module):
    ...
    def forward(self, batch):
        x, mask, lengths = batch["x"], batch["mask"], batch["lengths"]
        bs = len(x)
        x = x.permute((0, 3, 1, 2))
        x = augment_x(x, mask, lengths, self.concatenate_time)
...
class Decoder_GRU(nn.Module):
    ...
    def forward(self, batch):
        ...
        z = augment_z(z, mask, lengths, self.concatenate_time)
        # Model
