...
class DDPMTrainer(object):
    ...
    def generate_batch(self, m_lens, dim_pose):
        xf_proj, xf_out = None, None #self.encoder.encode_text(caption, self.device)
        
        B = len(m_lens) #len(caption)
    ...
    def load(self, model_dir):
        ...
        print("Loaded checkpoint: ", model_dir)
        return checkpoint['ep'], checkpoint.get('total_it', 0)

    def train(self, train_loader):
        rank = 0 #rank, world_size = get_dist_info()
        self.to(self.device)
...
if __name__ == "__main__":
    from options import TrainCompOptions
    ...
    dataset_config = {
        "name": "mocap_unannot",
        "root": '/media/mynewdrive/datasets/dannce/social_rat/SCN2A_WK1',
        "seqlen": 64,
        "batch_size": 256
    }
    dataloaders = prepare_dataloaders(dataset_config)
