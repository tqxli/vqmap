...
from src.recognition.models.classifer import load_classifier


class Evaluation:
    def __init__(self, dataname, parameters, device, seed=None):
        layout = "smpl" if parameters["glob"] else "smpl_noglobal"
        model = load_classifier(parameters)

        model = model.to(parameters["device"])

        modelpath = "exp/classifier/checkpoint_0500.pth.tar"
    ...
    def evaluate(self, model, loaders):
        def print_logs(metric, key):
            print(f"Computing {metric} on the {key} loader ...")
        ...
        for sets in ["train"]:
            ...
            for key, loaderSets in loaders.items():
                ...
                metrics[f"diversity_{key}"], metrics[f"diversity_by_action_{key}"], metrics[f"multimodality_{key}"] = ret
