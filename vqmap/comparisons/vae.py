...
class VAE(nn.Module):
    ...
    def _data_preproc(self, batch):
        ...
        batch["x"] = x.reshape(bs, t, n_joints, 3).permute(0, 2, 3, 1)
...
def get_vae(architecture, nfeats, latent_dim, lambdas, num_frames=64):
    ...
    encoder = encoder_cls(
        'Encoder', n_joints, n_dim, num_frames, latent_dim=latent_dim
    )
    decoder = decoder_cls(
...
if __name__ == "__main__":
    ...
    encoder = Encoder_GRU(
        'Encoder', n_joints, nfeats, num_frames, latent_dim=latent_dim
    )
    decoder = Decoder_GRU(
        'Decoder', n_joints, nfeats, num_frames, latent_dim=latent_dim
    )
    bs = 10
    # lengths = (torch.ones(bs) * num_frames).long()
    # mask = lengths_to_mask(lengths)
    # batch = {
    #     "x": torch.randn(bs, n_joints, nfeats, num_frames),
    #     "mask": mask,
    #     "lengths": lengths
    # }
    batch = {"motion": torch.randn(bs, num_frames, n_joints*nfeats)}
    lambdas = {
        "recons": 1.0,
        "kl": 0.0001
    }
