...
class ModelViewer(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        desc = QLabel("Decoded kinematic patterns from each discrete VQ-MAP code")
        ...
        add_label_btn.setProperty("class", "warning")
        # add_label_btn.setMaximumWidth(120)
        add_label_btn.clicked.connect(self.create_new_label)
        header_row.addWidget(add_label_btn)

        # Apply annotation button
        apply_button = QPushButton("Assign label to selected code")
    ...
    def extract_and_display_codes(self):
        ...
        for i in range(self.model.N):
            for j in range(self.model.M):
                ...
                title.setStyleSheet("font-size: 8pt;")
                cell_layout.addWidget(title)

                # Create button
                btn = CodeButton(i, j, self)
                btn.setObjectName("grid_cell")
    ...
    def select_codes_with_label(self, label):
        ...
        if codes:
            ...
            QMessageBox.information(
                self, "Selection", f"Found {count} codes with label '{label}'"
            )
    ...
    def save_annotations(self):
        ...
        try:
            ...
            data = {
                "annotations": {
                    f"{i},{j}": label for (i, j), label in self.annotations.items()
                },
    ...
    def load_annotations(self):
        ...
        if not os.path.exists(filename):
            QMessageBox.warning(
                self, "Not Found", "No annotations file found in this directory"
            )
            return
    ...
    def export_annotations(self):
        ...
        try:
            ...
            QMessageBox.information(
                self, "Exported", f"Annotations exported to {filename}"
            )
